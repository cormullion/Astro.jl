...
@test(yr == 1957)
@test(mo == 10)
@test isapprox(day, 4.81, atol=0.01)
println(" passed")
...
@test isapprox(diff_angle(deg2rad(10), deg2rad(0))   |> rad2deg, 350, atol=.1)
@test isapprox(diff_angle(deg2rad(0), deg2rad(10))   |> rad2deg, 10,  atol=.1)
@test isapprox(diff_angle(deg2rad(-10), deg2rad(10)) |> rad2deg, 20,  atol=.1)
println(" passed")
...
@test isapprox(hangle_to_dec_deg(23,55,55), 358.9791666666667, atol=0.0001)
println(" passed")
...
@test isapprox(y, 0.876125, atol=1e-6)
@test isapprox(interpolate_angle3(1, [0, 30,60]), 34.86725, atol=0.01)
@test isapprox(interpolate_angle3(0, [0, 30,60]), 30.0,     atol=0.01)
@test isapprox(interpolate_angle3(-1, [0, 30,60]), 25.13,   atol=0.01)
println(" passed")

print("polynomials")
@test @polynomial_horner(18, 7, 4, 1) == 403
@test polynomial([7, 4, 1], 18) == 403
@test isapprox(@polynomial_horner(18, 9, 13, 7.4, 4, 1), 130944.59, atol=.1)
@test isapprox(polynomial((9, 13, 7.4, 4, 1), 18), 130944.6, atol=.1)
println(" passed")
